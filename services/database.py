...
async def a_handle_execution(stmt):
    with engine.begin() as connection:
        try:
            connection.execute(stmt)
        except Exception as e:
...
async def a_get_full_response(model:str, method: str, properties:dict = None):
    result = {'data': []}
    if not properties:
        properties = {}
    data = {
...
async def a_get_response(model:str, method: str, properties:dict = {}, url: str = url):
    data = {
        "apiKey": api_key,
        "modelName": model,
        "calledMethod": method,
